...
usage() {
  cat <<'USAGE'
Usage: scripts/linux/p2/verify-network-parity.sh --root-disk PATH --firmware PATH [options]
...
"$script_dir/verify-multinic-model.sh" --mode host-tap "${common_args[@]}" --name p2-parity-hosttap2

section "CNI provider: 1 NIC"
"$script_dir/verify-cni-provider.sh" "${common_args[@]}" --name p2-parity-cni1

section "CNI provider: 2 NICs"
"$script_dir/verify-multinic-model.sh" --mode cni "${common_args[@]}" --name p2-parity-cni2
