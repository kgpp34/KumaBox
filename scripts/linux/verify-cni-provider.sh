...
usage() {
  cat <<'USAGE'
Usage: scripts/linux/verify-cni-provider.sh --root-disk PATH --firmware PATH [options]

Verifies P2-08 CNI provider state handling with a mock CNI plugin. The script
checks CNI ADD result parsing, provider index persistence, VM networkConfigs,
CNI DEL cleanup, and pending cleanup on DEL failure. It does not start a VM.
...
"${cat_cmd[@]}" "$config_file" | jq '.nets'

section "delete VM and verify CNI DEL"
"${kumabox_cmd[@]}" --config "$config_path" delete "${name}-ok" --force
if ! "${cat_cmd[@]}" "$cni_log" | grep -q "DEL $vm_id $tap /proc/self/ns/net"; then
