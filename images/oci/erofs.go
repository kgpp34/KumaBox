...
type EROFSConverter struct {
	architecture string
	limits       Limits
}

func NewEROFSConverter(ctx context.Context, architecture string) (*EROFSConverter, error) {
	return NewEROFSConverterWithLimits(ctx, architecture, DefaultLimits())
}

func NewEROFSConverterWithLimits(ctx context.Context, architecture string, limits Limits) (*EROFSConverter, error) {
	if !limits.Valid() || (architecture != "amd64" && architecture != "arm64") {
		return nil, invalidSource("invalid converter architecture or size limits")
	}
	...
	return &EROFSConverter{architecture: architecture, limits: limits}, nil
}

func (c *EROFSConverter) Convert(ctx context.Context, descriptor images.Descriptor, source io.Reader, workDir string) (images.ConvertedLayer, error) {
	...
	if scanErr == nil {
		if _, err := io.Copy(io.Discard, stream); err != nil {
			scanErr = fmt.Errorf("drain OCI layer: %w", err)
		}
	}
	if scanErr != nil {
		cancel()
	...
	if err := errors.Join(scanErr, closeErr, waitErr, ctx.Err()); err != nil {
		return images.ConvertedLayer{}, sourceError(err)
	}
...
func scanBoot(source io.Reader, workDir, architecture string, limit int64) ([]images.StagedBootFile, []string, bool, error) {
	...
	for {
		...
		if err != nil {
			return nil, nil, false, invalidSource("read layer tar: %v", err)
		}
		clean := filepath.ToSlash(filepath.Clean(header.Name))
		if filepath.IsAbs(header.Name) || clean == ".." || strings.HasPrefix(clean, "../") {
			return nil, nil, false, invalidSource("unsafe layer path %q", header.Name)
		}
...
func writeBootFile(source io.Reader, destination string, decompressKernel bool, limit int64) error {
	...
	if written == 0 || written > limit {
		return invalidSource("boot artifact size %d is outside limit %d", written, limit)
	}
	return nil
}
...
func digestPath(ctx context.Context, path string) (images.Digest, int64, error) {
	...
	hash := sha256.New()
	size, copyErr := io.Copy(hash, &contextInput{ctx: ctx, source: file})
	closeErr := file.Close()
