...
|---|---|---|---|
| CLI 生命周期 | `vm create/run/start/stop/list/inspect/console/exec/logs/rm/status` | 已有 create/start/stop/ps/inspect/console/exec/rm；logs 下一步 | 命令行为逐项对齐；KumaBox 当前不加 `vm` 中间层 |
...
| Logs | persistent per-VM log，tail/follow/reopen，delete 清理 log dir | log 已持久化，CLI 与 rm cleanup 未实现 | 下一条命令同时补齐读取和归属清理 |
...
| CNI network | reserve → netns → NIC intents → ADD/TAP/TC；stop quiesce/start recover/rm DEL | 未实现 | `logs` 后立即进入网络切片 |
